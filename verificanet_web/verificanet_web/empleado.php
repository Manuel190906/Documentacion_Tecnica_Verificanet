...
$stmt = $conn->prepare("
    SELECT i.*, c.nombre as cliente_nombre, c.empresa 
    FROM incidencias i 
...
$stmt_stats = $conn->prepare("
    SELECT 
        SUM(CASE WHEN estado != 'Resuelto' AND estado != 'Cerrado' THEN 1 ELSE 0 END) as pendientes,
        SUM(CASE WHEN estado = 'Resuelto' OR estado = 'Cerrado' THEN 1 ELSE 0 END) as resueltas
    FROM incidencias 
    WHERE id_empleado = ?
");
                        ...
                        <div class="grid-2">
                            <div class="form-group">
                                ...
                                <select name="id_cliente" id="cliente" required>
                                    ...
                                    <?php
                                    $clientes = $conn->query("SELECT id_cliente, nombre, empresa FROM clientes ORDER BY nombre");
                                    while ($cliente = $clientes->fetch_assoc()):
                                    ?>
                                        <option value="<?php echo $cliente['id_cliente']; ?>">
                                            <?php echo htmlspecialchars($cliente['nombre'] . ' - ' . $cliente['empresa']); ?>
                                        </option>
                                    <?php endwhile; ?>
                                </select>
                            </div>
                                ...
                                <tbody>
                                    <?php while ($inc = $incidencias->fetch_assoc()): ?>
                                        <tr>
                                            ...
                                            <td><?php echo htmlspecialchars($inc['empresa']); ?></td>
                                            <td>
