...
if ($id_cliente) {
    ...
    ");
    if ($res) { while ($r = $res->fetch_assoc()) { $contratos[] = $r; } }
}
...
if ($id_cliente) {
    $res = $conn->query("
        ...
        LIMIT 10
    ");
    if ($res) { while ($r = $res->fetch_assoc()) { $incidencias[] = $r; } }
}
...
<head>
    ...
    <style>
        .servicios-grid { display:grid; grid-template-columns:repeat(auto-fill, minmax(260px,1fr)); gap:24px; margin-top:20px; }
        .servicio-card { background:#fff; border:1px solid #dbeafe; border-radius:12px; overflow:hidden; box-shadow:0 2px 8px rgba(59,130,246,0.08); }
        .servicio-header { background:linear-gradient(135deg,#3b82f6,#60a5fa); padding:20px; }
        .servicio-header h3 { color:#fff; font-size:16px; margin:0; }
        .servicio-body { padding:20px; }
        .servicio-body p { color:#374151; font-size:14px; margin-bottom:8px; }
        .servicio-precio { font-size:22px; font-weight:700; color:#1e40af; margin:10px 0; }
        ...
        .contrato-item { background:#f0f9ff; border:1px solid #bae6fd; border-radius:10px; padding:16px; margin-bottom:12px; }
        .contrato-item h4 { color:#0369a1; margin:0 0 8px; }
        .contrato-item p { color:#374151; font-size:14px; margin:4px 0; }
        .tabs { display:flex; gap:10px; margin-bottom:24px; flex-wrap:wrap; }
        .tab { padding:10px 22px; border-radius:8px; text-decoration:none; color:#1e40af; border:2px solid #bfdbfe; background:#fff; font-weight:500; transition:all 0.2s; }
